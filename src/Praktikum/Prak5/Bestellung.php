    ...
    protected function generateView(): void
    {
        ...
        <body>
            <nav>
                ...
                <a href="Bestellung.php" class="active">Bestellung</a>
        ...
        foreach ($articleList as $article) {
            echo <<<HTML
                <div class="pizza-item">
                    <img class="pizza-image" width="150" height="100" src="{$article['picture']}" alt="{$article['name']}" title="{$article['name']}" data-price="{$article['price']}">
                    <p>{$article['name']}</p>
                    <p>{$article['price']}</p>
                </div>
            HTML;
        ...
        echo <<<HTML
            ...
            <form action="Bestellung.php" method="post" accept-charset="UTF-8">
                <select name="warenkorb[]" id="selectPizza" class="selectPizza" onchange="seePrice()" size="7" multiple></select>
                <p class="priceTotal">Total Price: €</p>
                <input type="hidden" name="totalPrice" id="totalPrice" value="0.00">
                <div>
                    <input type="text" id="inputAddress" name="address-input" placeholder="Ihre Adresse" size="20">
                </div>
                <div>
                    <button type="reset" onclick="clearAllSelectOption()">Alle Löschen</button>
                    <button type="button" onclick="clearSelectOption()">Auswahl Löschen</button>
